...
let reRenderSkyBox = () => {
  ...
  let cameraMatrix = lookAt(cameraPos, add(cameraPos, cameraFront) as Vec3, VEC_Y)
  let viewMatrix = inverse(cameraMatrix)
  let temp = mult(preCalculatedCPM, viewMatrix)
  helper.prepare({
    attributes: [
    ...
    uniforms: [
      ...
      { varName: 'modelViewMatrix', data: flatten(inverse(temp as Mat)), method: 'Matrix4fv' },
